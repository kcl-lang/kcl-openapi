...
// TestSplitYamlStreamWithLeadingComment reproduces the file layout used by the
// kube-prometheus-stack helm chart: a URL attribution comment followed by
// `---` and then a single CRD document. The leading comment must not be
// returned as its own document or the downstream decoder reports
// `Object 'Kind' is missing in '# ...'`. See https://github.com/kcl-lang/kcl-openapi/issues/147
func TestSplitYamlStreamWithLeadingComment(t *testing.T) {
	crds := `# https://raw.githubusercontent.com/prometheus-operator/prometheus-operator/v0.93.1/example/prometheus-operator-crd/monitoring.coreos.com_servicemonitors.yaml
---
apiVersion: apiextensions.k8s.io/v1
kind: CustomResourceDefinition
metadata:
...
spec:
  ...
  names:
    ...
    kind: ServiceMonitor
`
	files, err := splitDocuments(crds)
	if err != nil {
		t.Fatalf("splitDocuments returned error: %v", err)
	}
	if len(files) != 1 {
		t.Fatalf("splitDocuments failed. expected 1, got %d: %q", len(files), files)
	}
	if !strings.Contains(files[0], "kind: CustomResourceDefinition") {
		t.Fatalf("splitDocuments dropped the CRD document: %q", files[0])
	}
	// And the leading comment must NOT appear as its own document.
	for _, f := range files {
		if !strings.Contains(f, "apiVersion") && strings.Contains(f, "#") {
			t.Fatalf("splitDocuments returned a comment-only document: %q", f)
		}
	}
}

// TestSplitDocumentsTrailingCommentOnly covers a file that ends with a
// trailing comment after the last separator (or with no separator at all).
// Such trailing content must not be emitted as a document.
func TestSplitDocumentsTrailingCommentOnly(t *testing.T) {
	// Each case uses double-quoted strings so the literal `\n` is a real
	// newline — previous raw-string inputs put a literal "\n" on the same
	// line, defeating the multi-line intent of these cases.
	cases := map[string]string{
		"trailing comment after CRD": v1Crd + "# generated by something\n# do not edit\n",
		"only comments":              "# only a comment\n# and another one\n",
		"only whitespace":            "\n\n   \n\t\n",
	}
	for name, in := range cases {
		t.Run(name, func(t *testing.T) {
			files, err := splitDocuments(in)
			if err != nil {
				t.Fatalf("splitDocuments returned error: %v", err)
			}
			for _, f := range files {
				if !strings.Contains(f, "apiVersion") {
					t.Fatalf("splitDocuments emitted a document without apiVersion: %q", f)
				}
			}
		})
	}
