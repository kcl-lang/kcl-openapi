...
func DefaultFuncMap(lang *LanguageOpts) template.FuncMap {
	return template.FuncMap(map[string]interface{}{
		...
		"toKCLValue":    toKCLValue,
		"escapeKeyword": escapeModelName,
	})
}
...
// FuncMap functions
func toKCLValue(data interface{}) string {
	if data == nil {
		return "None"
	}
	...
	case reflect.Slice:
		...
		if dataSlice, ok := data.(yaml.MapSlice); ok {
			...
			for _, v := range dataSlice {
				k := v.Key
				v := v.Value
				dictContents = append(dictContents, fmt.Sprintf("%s: %s", toKCLValue(k), toKCLValue(v)))
			}
		...
		for i := 0; i < value.Len(); i++ {
			sliceContents = append(sliceContents, toKCLValue(value.Index(i).Interface()))
		}
