...
// escapeModelName add "$" prefix to name if it is conflict with KCL keyword
func escapeModelName(modelName string) string {
	kclKeywords := []string{
		"import",
		"as",
		"rule",
...
func (t *typeResolver) kclTypeName(modelName string) string {
	escapedName := escapeModelName(modelName)
